...
namespace GUtilsUnity.Randomization.Extensions
{
    ...
    {
        /// <summary>
        /// Creates a new <see cref="Vector2Int"/> using the provided <see cref="IRandomGenerator"/>
        /// in the range [int.MinValue, int.MaxValue]
        /// </summary>
        /// <param name="randomGenerator">The random number generator to use.</param>
        /// <returns>A new instance of <see cref="Vector2Int"/> with randomly generated x and y values
        /// in the range [int.MinValue, int.MaxValue]</returns>
        public static Vector2Int NewVector2Int(this IRandomGenerator randomGenerator)
        {
            return new Vector2Int(randomGenerator.NewInt(), randomGenerator.NewInt());
        }

        /// <summary>
        /// Creates a new <see cref="Vector2Int"/> using the provided <see cref="IRandomGenerator"/> and
        /// specified range for x and y values.
        /// </summary>
        /// <param name="randomGenerator">The random number generator to use.</param>
        /// <param name="minInclusive">The minimum inclusive values for x and y.</param>
        /// <param name="maxExclusive">The maximum exclusive values for x and y.</param>
        /// <returns>A new instance of <see cref="Vector2Int"/> with randomly generated x and y values
        /// within the specified range.</returns>
        public static Vector2Int NewVector2Int(this IRandomGenerator randomGenerator, Vector2Int minInclusive, Vector2Int maxExclusive)
        {
            return new Vector2Int(
                randomGenerator.NewInt(minInclusive.x, maxExclusive.x),
                randomGenerator.NewInt(minInclusive.y, maxExclusive.y)
            );
        }
        ...
        {
            normalizedPercentage = Mathf.Clamp(normalizedPercentage, 0f, 1f);
